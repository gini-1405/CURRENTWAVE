...
function TopStories(props) {
    ...
    useEffect(() => {
        document.title = `${props.section === "home" ? '' : section} Headlines - CurrentWave`
        async function fetchData() {
            try {
                ...
                document.title = `CurrentWave - ${props.section === "home" ? '' : section} Headlines`
            }
            catch (error) {
                console.log(error)
            }
        ...
        <>
            {articles.length === 0 ? <Spinner /> :
                <div className="container my-3" style={{ padding: "4rem" }}>
                    <h2 id="heading" className="text-center">CurrentWave - {props.section === "home" ? '' : section} Headlines</h2>
                    {
                        <InfiniteScroll
                            pageStart={0}
